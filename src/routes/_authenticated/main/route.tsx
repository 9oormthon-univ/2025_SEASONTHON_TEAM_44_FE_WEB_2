...
import { FILTER_OPTIONS, FILTER_LABELS } from './const';
...
export const Route = createFileRoute('/_authenticated/main')({
  ...
  validateSearch: (search) => ({
    filter: (search.filter as string) || FILTER_OPTIONS.NEWEST_REGISTER,
...
const MainPage = () => {
  ...
  const { filter: selectedFilter, search: searchQuery } = Route.useSearch();
  ...
  const handleSelectFilter = (filter: string) => {
    navigate({
      search: { filter, search: searchQuery || '' },
    });
